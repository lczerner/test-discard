 ...
 *
 * usage: 
 * <program> [-s start] [-r record_size] [-t total_size] [-d device]
 *
 * You can specify start, record_size and total_size like ordinary 
 * number, or you can add either: 
 * 	k|K - kilobytes (n*1024)
 * 	m|M - megabytes (n*1024*1024)
 * 	g|G - gigabytes (n*1024*1024*1024)
 * 	
 * For example: <program> -s 10k -r 4k -t 10M -d /dev/sdb1
 * start : 10240
 * record_size : 4096
 * total_size : 10485760
 *
...
int main (int argc, char **argv) {
	...
	for (unsigned long long i = 1;i <= repeat;i++) {
		...
		if ((defs.start + defs.total_size) > dev_size) {
			fprintf(stderr,"Boundaries does not fit to the device\n");
			return EXIT_FAILURE;
		}
